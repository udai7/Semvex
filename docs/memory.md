...
> **UPDATE 2026-07-08 — items 1, 2, 4, 5 below are DONE.** HF `hf` provider live-tested & fixed (legacy `api-inference.*` host is dead; now uses the router `.../pipeline/feature-extraction` endpoint — verified an HF-embedded query returns identical results to the locally-embedded catalog). Real ESCI ingest run: **25,040 products in Neon** (24.5k ESCI `us` + 40 demo), all real **bge-small** embeddings, **156 MB** (safe under 500 MB free). Whole catalog (incl. the 40 demo rows) re-embedded to one vector space; semantic clearly beats keyword. Fixed a 10× ingest perf bug (`upsert_batch` was 500 round-trips/batch → now one multi-row upsert). Frontend `/search` gained a live `/health` engine/provider status strip + a fixed badge bug. **Run ingest with `OMP_NUM_THREADS=2 MKL_NUM_THREADS=2`** or torch stalls on this box. Remaining: item 3 (dim ≠ 384 — n/a, staying on bge-small 384), the 2 skipped tests (need bge-small on the LOCAL test pg, not Neon), and infra items 6–8.
